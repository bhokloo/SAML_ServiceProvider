    ...
    public class SamlController : Controller
    {
            ...
            if (Response != null)
            {
                try
                {
                   ...
                   out relayState);
                    ...
                    var userIdentity = applicationUserManager.CreateIdentity(applicationUser, DefaultAuthenticationTypes.ApplicationCookie);

                    authenticationManager.SignIn(new AuthenticationProperties() { AllowRefresh = true, IsPersistent = false }, userIdentity);
                    

                    if (!string.IsNullOrEmpty(relayState))
                    {
                        var accessToken = new ActivantsSpController().Get(applicationUser.UserName).Result;
                        var accessToken = applicationUser.UserName;
                        ...
                        {
                            return Redirect(ClientUrl + responseFromSp + "?access_token=" + accessToken);
                        }
                        else
                        {
        ...
        public ActionResult InitiateSingleLogout(string relayState = null)
        {
            try
            {
                ...
                if (Request.QueryString["responseForLogout"] != null)
                {
                    relayState = Request.QueryString["responseForLogout"];
            ...
            catch (Exception e)
            {
                if (Request.QueryString["responseForLogout"] != null)
                {
                    var fullUrl = Decrypt(HttpUtility.UrlDecode(Request.QueryString["responseForLogout"]));
                    string[] Url = fullUrl.Split(new string[] { "&" }, StringSplitOptions.None);
                    var ReturnUrl = Url[0];
                    var errUrl = Url[2];
                    return Redirect(ReturnUrl + errUrl);
        ...
        {
            if (url.Contains("http://118.201.3.45:302/") || url.Contains("https://localhost:44313/") || url.Contains("https://192.168.1.172:44333/") || url.Contains("http://localhost:8090"))
                return true;
            else
                return false;
        }
